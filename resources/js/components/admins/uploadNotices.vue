...
<!-- -------------- Notice Type ---------------------- -->
    <div class="form-group" id="noticeType">
                <label for="Type">Notice Type</label>               
                <select v-model="type" class="form-control shadow-none"  name="noticeType"  @change="setLinkPost()">
...
<!-- ----------------- Link URL -------------------- -->
          ...
          <input v-model="link" type="text" class="form-control shadow-none" id="linkurl" name="linkurl" 
          placeholder="Enter URL extension for Post"  autocomplete="off" :disabled="Disable">        
        </div><br/>


    <div class="large-12 medium-12 small-12 cell">
      <v-btn  color="light-green white--text float-left" v-on:click.stop="submitFiles(); closeDialog()">Submit</v-btn>
    </div>
    ...
    methods: {
      ...
      submitFiles(){
        ...
        for( var i = 0; i < this.files.length; i++ ){
         ...
         this.$store.dispatch('noticeStore/POST_NOTICE', formData);
